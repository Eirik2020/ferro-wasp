...
pub enum OutputProfile {
    StaticPwmBringup,
    FlightPwm,
    FlightDshotFourMotorBench,
    FlightDshot,
    WaveformDemoDeferred,
}

pub const DEFAULT_OUTPUT_PROFILE: OutputProfile = OutputProfile::FlightDshot;

pub const PWM_DMA_NOTE: &str =
    "Four-motor DShot600 is the FCU3 default; four-channel PWM remains an explicit fallback.";
...
pub struct DshotFourMotorProfile {
    ...
    pub prearm_stop_hold_ms: u32,
    pub idle_throttle_command: u16,
    pub idle_qualification_min_erpm_div100: u16,
    pub idle_qualification_max_erpm_div100: u16,
    pub idle_qualification_spinup_grace_ms: u32,
    pub idle_qualification_timeout_ms: u32,
    pub idle_qualification_max_sample_age_ms: u32,
    pub idle_qualification_consecutive_samples: u8,
}
...
pub const DSHOT_FOUR_MOTOR_PROFILE: DshotFourMotorProfile = DshotFourMotorProfile {
    ...
    // Stop frames already run continuously while disarmed. This short dwell is
    // a safety recheck window, not a PWM-style ESC arming requirement.
    prearm_stop_hold_ms: 100,
    // Preserve the target-proven command for the first tuning checkpoint.
    // The DShot transport maps command 65 to protocol value 112.
    idle_throttle_command: 65,
    // Powered props-off evidence measured 6,500-7,200 eRPM at idle. The
    // qualification window leaves spin-up and unit-to-unit margin while
    // rejecting a stopped lane and an unexpectedly fast lane.
    idle_qualification_min_erpm_div100: 30,
    idle_qualification_max_erpm_div100: 100,
    idle_qualification_spinup_grace_ms: 250,
    idle_qualification_timeout_ms: 1_200,
    idle_qualification_max_sample_age_ms: 200,
    idle_qualification_consecutive_samples: 3,
};

const _: () = {
    assert!(DSHOT_FOUR_MOTOR_PROFILE.idle_throttle_command <= DSHOT_IDLE_TUNING_MAX_COMMAND);
    assert!(DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_min_erpm_div100 > 0);
    assert!(
        DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_min_erpm_div100
            < DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_max_erpm_div100
    );
    assert!(
        DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_spinup_grace_ms
            < DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_timeout_ms
    );
    assert!(DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_consecutive_samples > 0);
};
...
mod tests {
    ...
    fn dshot_profile_preserves_all_fcu3_motor_pins_and_routes() {
        ...
        assert_eq!(DSHOT_FOUR_MOTOR_PROFILE.prearm_stop_hold_ms, 100);
        assert_eq!(DSHOT_FOUR_MOTOR_PROFILE.idle_throttle_command, 65);
        assert_eq!(
            DSHOT_FOUR_MOTOR_PROFILE.idle_qualification_min_erpm_div100,
            30
        );
        ...
        assert_eq!(DEFAULT_OUTPUT_PROFILE, OutputProfile::FlightDshot);
        assert!(PWM_DMA_NOTE.contains("PWM remains an explicit fallback"));
    }
}

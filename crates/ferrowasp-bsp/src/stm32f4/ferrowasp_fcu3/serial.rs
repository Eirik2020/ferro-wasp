...
impl SerialRoute {
    pub fn validate_profile(self, profile: SerialProfile) -> Result<(), SerialRouteError> {
        ...
        if profile.protocol.requires_tx() && !self.capabilities.tx {
...
trait SerialProtocolExt {
    fn requires_tx(self) -> bool;
}

impl SerialProtocolExt for SerialProtocol {
    fn requires_tx(self) -> bool {
...
mod tests {
    ...
    #[test]
    fn active_serial_profiles_validate_against_capabilities() {
    ...
    #[test]
    fn unsupported_protocols_are_rejected_before_hardware_init() {
        assert_eq!(
            USART2_SBUS.validate_profile(SerialProfile::msp()),
            Err(SerialRouteError::UnsupportedProtocol)
        );
        assert_eq!(
            UART4_MSP.validate_profile(SerialProfile::sbus()),
            Err(SerialRouteError::UnsupportedProtocol)
        );
    }
}

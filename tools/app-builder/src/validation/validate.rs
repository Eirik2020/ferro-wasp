...
#[cfg(test)]
mod tests {
    use crate::validation::manifest::parse;
    ...
    fn incompatible_schema_versions_are_rejected() {
        let application = error_for(|manifest| manifest.application_schema_version = 7);
        assert!(application.contains("application.schema_version"));

        let bsp = error_for(|manifest| manifest.bsp_schema_version = 5);
        assert!(bsp.contains("bsp.schema_version"));
    }

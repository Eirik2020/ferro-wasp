...
pub enum ComponentInitLocalResource {
    ...
    /// Private storage synthesized for one conceptual observer output.
    ObserverOutput {
        /// Stable portable type ID of the observed value.
        type_id: &'static str,
    ...
    /// Private storage synthesized for one conceptual safety output.
    SafetyOutput {
        /// Stable portable type ID of transferred messages.
        type_id: &'static str,
        ...
        /// Number of messages that callers may enqueue without a dequeue.
...
pub enum ComponentResourceKind {
    ...
    /// One non-consuming latest-value output with synthesized publisher and reader handles.
    ObserverOutput {
        /// Stable portable type ID of the observed value.
        type_id: &'static str,
    ...
    /// One authoritative bounded SPSC output with synthesized producer and consumer handles.
    SafetyOutput {
        /// Stable portable type ID of transferred messages.
        type_id: &'static str,
...
/// Declares one conceptual latest-value observer output.
pub const fn observer_output(
    id: &'static str,
    type_id: &'static str,
    rust_type: &'static str,
) -> ComponentResource {
    ComponentResource {
        id,
        kind: ComponentResourceKind::ObserverOutput { type_id, rust_type },
        activation: ComponentActivation::Always,
    }
}

/// Declares one conceptual authoritative safety output.
pub const fn safety_output(
    id: &'static str,
    type_id: &'static str,
    rust_type: &'static str,
    usable_capacity: usize,
    ...
    ComponentResource {
        id,
        kind: ComponentResourceKind::SafetyOutput {
            type_id,
            rust_type,
            usable_capacity,
        },
...
/// Selects which synthesized endpoint a component task binds.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ComponentTaskBindingEndpoint {
    /// Bind an ordinary external or internal resource directly.
    Direct,

    /// Bind the private publisher of a conceptual observer output.
    ObserverPublisher,

    /// Bind the exposed reader of a conceptual observer output.
    ObserverReader,

    /// Bind the private producer of a conceptual safety output.
    SafetyProducer,

    /// Bind the exposed consumer of a conceptual safety output.
    SafetyConsumer,
}
...
pub struct ComponentTaskBinding {
    ...
    /// Direct resource or synthesized channel endpoint selected by this binding.
    pub endpoint: ComponentTaskBindingEndpoint,
}
...
impl ComponentTaskBinding {
    ...
    pub const fn new(task_resource: &'static str, component_resource: &'static str) -> Self {
        Self {
            task_resource,
            component_resource,
            endpoint: ComponentTaskBindingEndpoint::Direct,
        }
    }

    /// Binds a task-local field to an observer output's private publisher.
    pub const fn observer_publisher(
        ...
        Self {
            task_resource,
            component_resource,
            endpoint: ComponentTaskBindingEndpoint::ObserverPublisher,
        }
    }

    /// Binds a task field to an observer output's exposed reader.
    pub const fn observer_reader(
        task_resource: &'static str,
        component_resource: &'static str,
    ) -> Self {
        Self {
            task_resource,
            component_resource,
            endpoint: ComponentTaskBindingEndpoint::ObserverReader,
        }
    }

    /// Binds a task-local field to a safety output's private producer.
    pub const fn safety_producer(
        task_resource: &'static str,
        component_resource: &'static str,
    ) -> Self {
        Self {
            task_resource,
            component_resource,
            endpoint: ComponentTaskBindingEndpoint::SafetyProducer,
        }
    }

    /// Binds a task-local field to a safety output's exposed consumer.
    pub const fn safety_consumer(
        task_resource: &'static str,
        component_resource: &'static str,
    ) -> Self {
        Self {
            task_resource,
            component_resource,
            endpoint: ComponentTaskBindingEndpoint::SafetyConsumer,
        }
    }
}
...
pub struct ExpandedResourceBinding {
    ...
    /// Logical source retained when this binding was authored as a component port.
    pub source_component_port: Option<ExpandedComponentPortReference>,
}

/// Owned logical component-port reference retained for validation diagnostics.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExpandedComponentPortReference {
    /// Component instance named by the author.
    pub component: String,

    /// Exposed logical port named by the author.
    pub port: String,
}
...
pub enum ExpandedSoftwareResourceKind {
    ...
    /// Publisher synthesized for a conceptual observer output.
    ObserverPublisher {
        /// Stable portable type ID of the observed value.
        type_id: &'static str,
        /// Concrete Rust value type stored by the observer.
        rust_type: &'static str,
    },

    /// Reader synthesized for a conceptual observer output.
    ObserverReader {
        /// Stable portable type ID of the observed value.
        type_id: &'static str,
        /// Concrete Rust value type stored by the observer.
    ...
    /// Producer synthesized for a conceptual safety output.
    SafetyProducer {
        /// Stable portable type ID of transferred messages.
        type_id: &'static str,
        /// Concrete Rust message type stored by the channel.
    ...
    /// Consumer synthesized for a conceptual safety output.
    SafetyConsumer {
        /// Stable portable type ID of transferred messages.
        type_id: &'static str,
        /// Concrete Rust message type stored by the channel.
...
impl ExpandedSoftwareResourceKind {
    ...
    pub const fn capability(self) -> TaskResourceCapability {
        match self {
            ...
            Self::ObserverPublisher { type_id, .. } => {
                TaskResourceCapability::ObserverPublisher(type_id)
            }
            Self::ObserverReader { type_id, .. } => TaskResourceCapability::ObserverReader(type_id),
            Self::SafetyProducer { type_id, .. } => TaskResourceCapability::SafetyProducer(type_id),
            Self::SafetyConsumer { type_id, .. } => TaskResourceCapability::SafetyConsumer(type_id),
        }
    }
    ...
    pub fn rust_type(self) -> String {
        match self {
            ...
            Self::Component(resource) => resource.rust_type.to_owned(),
            Self::ObserverPublisher { rust_type, .. } => {
                format!("ObserverPublisher<'static, {rust_type}>")
            }
            Self::ObserverReader { rust_type, .. } => {
                format!("ObserverReader<'static, {rust_type}>")
            }
            Self::SafetyProducer { rust_type, .. } => {
                format!("SafetyProducer<'static, {rust_type}>")
            }
            Self::SafetyConsumer { rust_type, .. } => {
                format!("SafetyConsumer<'static, {rust_type}>")
            }
        }
    }
    ...
    pub fn initial_value(self, resource_id: &str) -> String {
        match self {
            ...
            Self::ObserverPublisher { .. }
            | Self::ObserverReader { .. }
            | Self::SafetyProducer { .. }
            | Self::SafetyConsumer { .. } => resource_id.to_owned(),
        }
    }
}
...
pub fn expand(board: &BoardDeclaration, app: &AppDeclaration) -> Result<ExpandedApp> {
    ...
    let mut expanded = ExpandedApp {
        tasks: app
            .tasks
            .iter()
            .map(|task| expand_standalone_task(app, task))
    ...
    for declaration in app.components {
        expand_component(app, declaration, &mut expanded)?;
    }
...
fn expand_standalone_task(
    app: &AppDeclaration,
    task: &crate::task::TaskDeclaration,
) -> Result<ExpandedTask> {
    Ok(ExpandedTask {
        id: task.id.to_owned(),
        ...
        local_resources: task
            .local_resources
            .iter()
            .map(|binding| expand_binding(app, binding))
            .collect::<Result<Vec<_>>>()?,
...
) -> Result<ExpandedResourceBinding> {
    Ok(ExpandedResourceBinding {
        task_resource: binding.task_resource().to_owned(),
        target: expand_resource_target(app, binding.target())?,
        source_component_port: match binding.target() {
            ResourceTarget::ComponentPort { component, port } => {
                Some(ExpandedComponentPortReference {
                    component: component.to_owned(),
                    port: port.to_owned(),
                })
            }
            ResourceTarget::Hardware(_) | ResourceTarget::Software(_) => None,
        },
    })
}

fn expand_resource_target(
    app: &AppDeclaration,
    target: ResourceTarget,
) -> Result<ExpandedResourceTarget> {
    match target {
        ResourceTarget::Hardware(id) => Ok(ExpandedResourceTarget::Hardware(id.to_owned())),
        ResourceTarget::Software(id) => Ok(ExpandedResourceTarget::Software(id.to_owned())),
        ResourceTarget::ComponentPort { component, port } => {
            let declaration = app
                .components
                .iter()
                .find(|declaration| declaration.id == component)
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "component-port reference names missing component instance `{component}`"
                    )
                })?;
            let resource = declaration
                ...
                .resources
                .iter()
                .find(|resource| resource.id == port)
                .ok_or_else(|| {
                    anyhow::anyhow!("component instance `{component}` has no port `{port}`")
                })?;
            if !resource.activation.active(declaration.configuration) {
                bail!("component instance `{component}` port `{port}` is inactive");
            }
            let generated_id = match resource.kind {
                ComponentResourceKind::InternalSoftware { visibility, .. } => {
                    if visibility != ComponentResourceVisibility::Exposed {
                        bail!("component instance `{component}` port `{port}` is private");
                    }
                    namespaced(component, port)
                }
                ComponentResourceKind::ObserverOutput { .. } => {
                    endpoint_id(&namespaced(component, port), "reader")
                }
                ComponentResourceKind::SafetyOutput { .. } => {
                    endpoint_id(&namespaced(component, port), "consumer")
                }
                ComponentResourceKind::ExternalHardware { .. }
                | ComponentResourceKind::ExternalSoftware { .. }
                | ComponentResourceKind::InitLocal(_) => {
                    bail!(
                        "component instance `{component}` resource `{port}` is not an exposed software port"
                    )
                }
            };
            Ok(ExpandedResourceTarget::Software(generated_id))
        }
    }
}
...
fn expand_component(
    ...
    let external_bindings = declaration
        ...
        .map(|binding| {
            Ok((
                ...
                expand_binding(app, binding)?.target,
            ))
        })
    ...
    for resource in declaration.definition.resources {
        ...
        match resource.kind {
            ...
            ComponentResourceKind::ObserverOutput { type_id, rust_type } => {
                expanded
                    .init_local_resources
                    .push(ExpandedInitLocalResource {
                        id: channel_storage_id(&id),
                        kind: ComponentInitLocalResource::ObserverOutput { type_id, rust_type },
                        owner_component: declaration.id.to_owned(),
                    });
                expanded
                    .software_local_resources
                    .push(ExpandedSoftwareResource {
                        id: endpoint_id(&id, "publisher"),
                        kind: ExpandedSoftwareResourceKind::ObserverPublisher {
                            type_id,
                            rust_type,
                        },
                        owner_component: Some(declaration.id.to_owned()),
                ...
                expanded
                    ...
                    .push(ExpandedSoftwareResource {
                        id: endpoint_id(&id, "reader"),
                        kind: ExpandedSoftwareResourceKind::ObserverReader { type_id, rust_type },
                        owner_component: Some(declaration.id.to_owned()),
                        owner_configuration: Some(declaration.configuration),
                        exposed: true,
                    });
            ...
            ComponentResourceKind::SafetyOutput {
                ...
                expanded
                    ...
                    .push(ExpandedInitLocalResource {
                        id: channel_storage_id(&id),
                        kind: ComponentInitLocalResource::SafetyOutput {
                            type_id,
                            rust_type,
                            usable_capacity,
                        },
                ...
                expanded
                    .software_local_resources
                    .push(ExpandedSoftwareResource {
                        id: endpoint_id(&id, "producer"),
                        kind: ExpandedSoftwareResourceKind::SafetyProducer { type_id, rust_type },
                        owner_component: Some(declaration.id.to_owned()),
                        owner_configuration: Some(declaration.configuration),
                        exposed: false,
                    });
                expanded
                    .software_local_resources
                    .push(ExpandedSoftwareResource {
                        id: endpoint_id(&id, "consumer"),
                        kind: ExpandedSoftwareResourceKind::SafetyConsumer { type_id, rust_type },
                        owner_component: Some(declaration.id.to_owned()),
                        owner_configuration: Some(declaration.configuration),
                        exposed: true,
                    });
        ...
        let resolve_bindings = |bindings: &[ComponentTaskBinding]| {
            bindings
                ...
                .map(|binding| {
                    ...
                    let target = match (component_resource.kind, binding.endpoint) {
                        (
                            ComponentResourceKind::ExternalHardware { .. }
                            | ComponentResourceKind::ExternalSoftware { .. },
                            ComponentTaskBindingEndpoint::Direct,
                        ) => external_bindings
                        ...
                        (
                            ComponentResourceKind::ObserverOutput { .. },
                            ComponentTaskBindingEndpoint::ObserverPublisher,
                        ) => ExpandedResourceTarget::Software(endpoint_id(
                            &namespaced(declaration.id, binding.component_resource),
                            "publisher",
                        )),
                        (
                            ComponentResourceKind::ObserverOutput { .. },
                            ComponentTaskBindingEndpoint::ObserverReader,
                        ) => ExpandedResourceTarget::Software(endpoint_id(
                            &namespaced(declaration.id, binding.component_resource),
                            "reader",
                        )),
                        (
                            ComponentResourceKind::SafetyOutput { .. },
                            ComponentTaskBindingEndpoint::SafetyProducer,
                        ) => ExpandedResourceTarget::Software(endpoint_id(
                            &namespaced(declaration.id, binding.component_resource),
                            "producer",
                        )),
                        (
                            ComponentResourceKind::SafetyOutput { .. },
                            ComponentTaskBindingEndpoint::SafetyConsumer,
                        ) => ExpandedResourceTarget::Software(endpoint_id(
                            &namespaced(declaration.id, binding.component_resource),
                            "consumer",
                        )),
                        (ComponentResourceKind::InitLocal(_), _) => {
                            bail!(
                        ...
                        _ => bail!(
                            "component `{}` task `{}` selects incompatible endpoint {:?} for resource `{}`",
                            declaration.id,
                            task.id,
                            binding.endpoint,
                            binding.component_resource
        ...
        let trigger = match task.trigger {
            ...
            ComponentTaskTrigger::Interrupt {
                ...
                let task_resource = task
                    ...
                    .find(|binding| {
                        binding.component_resource == resource
                            && binding.endpoint == ComponentTaskBindingEndpoint::Direct
                    })
...
fn validate_component_instances(board: &BoardDeclaration, app: &AppDeclaration) -> Result<()> {
    ...
    for component in declarations {
        for resource in component.definition.resources {
            if !resource.activation.active(component.configuration) {
                continue;
            }
            match resource.kind {
                ComponentResourceKind::InternalSoftware {
                    ...
                    software.insert(
                        ...
                        (
                            software_resource.capability(),
                            visibility == ComponentResourceVisibility::Exposed,
                        ),
                    );
                }
                ComponentResourceKind::ObserverOutput { type_id, .. } => {
                    software.insert(
                        endpoint_id(&namespaced(component.id, resource.id), "reader"),
                        (TaskResourceCapability::ObserverReader(type_id), true),
                    );
                }
                ComponentResourceKind::SafetyOutput { type_id, .. } => {
                    software.insert(
                        endpoint_id(&namespaced(component.id, resource.id), "consumer"),
                        (TaskResourceCapability::SafetyConsumer(type_id), true),
                    );
                }
                ComponentResourceKind::ExternalHardware { .. }
                | ComponentResourceKind::ExternalSoftware { .. }
    ...
    for declaration in declarations {
            ...
            match resource.kind {
                ...
                ComponentResourceKind::SafetyOutput {
                    ...
                    if usable_capacity == 0 {
                        bail!(
                            "component definition `{}` safety output `{}` requires a positive usable capacity",
                            declaration.definition.id,
                            resource.id
                        );
                    }
                    usable_capacity.checked_add(1).ok_or_else(|| {
                        anyhow::anyhow!(
                            "component definition `{}` safety output `{}` usable capacity overflows its backing queue length",
                            declaration.definition.id,
                            resource.id
                        )
                    })?;
        ...
        for binding in declaration.bindings {
            ...
            let target = expand_resource_target(app, binding.target())?;
            match (resource.kind, target) {
                (
                    ComponentResourceKind::ExternalHardware { capability },
                    ExpandedResourceTarget::Hardware(id),
                ) => {
                    let hardware = hardware.get(id.as_str()).ok_or_else(|| {
                        anyhow::anyhow!(
                ...
                (
                    ...
                    ExpandedResourceTarget::Software(id),
                ) => {
                    let (actual, exposed) =
                        software.get(id.as_str()).copied().ok_or_else(|| {
                            anyhow::anyhow!(
                    ...
                    if actual != capability {
                        bail!(
                            "component `{}` resource `{}` requires {capability:?}, but {} provides {actual:?}",
                            declaration.id,
                            resource.id,
                            resource_target_description(binding.target())
                        );
                    }
                    if !exposed {
                        bail!(
                ...
                (
                    ComponentResourceKind::ExternalHardware { .. },
                    ExpandedResourceTarget::Software(_),
                ) => {
                    bail!(
                ...
                (
                    ComponentResourceKind::ExternalSoftware { .. },
                    ExpandedResourceTarget::Hardware(_),
                ) => {
                    bail!(
...
fn endpoint_id(output_id: &str, endpoint: &str) -> String {
    format!("{output_id}_{endpoint}")
}

fn channel_storage_id(output_id: &str) -> String {
    endpoint_id(output_id, "channel")
}

fn resource_target_description(target: ResourceTarget) -> String {
    match target {
        ResourceTarget::Hardware(id) => format!("board resource `{id}`"),
        ResourceTarget::Software(id) => format!("software resource `{id}`"),
        ResourceTarget::ComponentPort { component, port } => {
...
fn validate_channel_type(
    ...
    if type_id.is_empty() {
        bail!(
            "component definition `{definition_id}` {role} `{resource_id}` has an empty portable type ID"
        );
    }
    syn::parse_str::<syn::Type>(rust_type).map_err(|_| {
        anyhow::anyhow!(
            "component definition `{definition_id}` {role} `{resource_id}` Rust type `{rust_type}` is invalid"
        )
    })?;
    Ok(())
}
...
mod tests {
    ...
    const CONCEPTUAL_SAFETY_COMPONENT: ComponentDefinition = ComponentDefinition {
        id: "conceptual_safety_link",
        layer: ComponentLayer::Functional,
        configuration_kind: ComponentConfigurationKind::None,
        tasks: &[ComponentTask {
            definition: SAFETY_PRODUCER_TASK,
            ...
            trigger: ComponentTaskTrigger::Spawned,
            parameters: &[],
            local_resources: &[ComponentTaskBinding::safety_producer(
                "output",
                "motor_command",
            )],
            shared_resources: &[],
    ...
    const CONCEPTUAL_SAFETY_CHANNEL: ComponentDeclaration = ComponentDeclaration {
        id: "control",
        definition: &CONCEPTUAL_SAFETY_COMPONENT,
        configuration: ComponentConfiguration::None,
        bindings: &[],
    };
    const CONCEPTUAL_SAFETY_CONSUMER: TaskDeclaration = SAFETY_CONSUMER_TASK
        .spawned_as("conceptual_actuator")
        .safety_class(TaskSafetyClass::SafetyCritical)
        .priority(2)
        .with_local(&[resource("input").to_component("control", "motor_command")]);
    ...
    fn command_input_observer_expands_to_private_publisher_shared_reader_and_storage() {
        ...
        const READER: TaskDeclaration = READER_DEFINITION
            .spawned_as("observer")
            .priority(1)
            .with_shared(&[resource("input").to_component("command_input", "rc_input")]);
        let expanded = expand(
            &F401_BOARD,
    ...
    #[test]
    fn conceptual_safety_output_synthesizes_capacity_and_owned_endpoints() {
        let expanded = expand(
            &F401_BOARD,
            &app(&[CONCEPTUAL_SAFETY_CONSUMER], &[CONCEPTUAL_SAFETY_CHANNEL]),
        )
        .unwrap();
        assert!(expanded.software_local_resources.iter().any(|resource| {
            resource.id == "control_motor_command_producer"
                && !resource.exposed
                && resource.kind.capability()
                    == TaskResourceCapability::SafetyProducer(SOFTWARE_MOTOR_CMD)
        }));
        assert!(expanded.software_local_resources.iter().any(|resource| {
            resource.id == "control_motor_command_consumer"
                && resource.exposed
                && resource.kind.capability()
                    == TaskResourceCapability::SafetyConsumer(SOFTWARE_MOTOR_CMD)
        }));
        assert!(expanded.init_local_resources.iter().any(|resource| {
            resource.id == "control_motor_command_channel"
                && matches!(
                    resource.kind,
        ...
        assert!(
            rendered
                .initialization
                .contains("cx.local.control_motor_command_channel.split()")
        );
    }

    #[test]
    fn conceptual_safety_output_rejects_zero_capacity_and_direct_binding() {
        const ZERO_CAPACITY: ComponentDefinition = ComponentDefinition {
            resources: &[safety_output(
                "motor_command",
                SOFTWARE_MOTOR_CMD,
                "MotorCmd",
            ...
            ..CONCEPTUAL_SAFETY_COMPONENT
        };
        const ZERO_CAPACITY_DECLARATION: ComponentDeclaration = ComponentDeclaration {
            definition: &ZERO_CAPACITY,
            ..CONCEPTUAL_SAFETY_CHANNEL
        };
        let error = expand(&F401_BOARD, &app(&[], &[ZERO_CAPACITY_DECLARATION])).unwrap_err();
        assert!(error.to_string().contains("positive usable capacity"));

        const DIRECT_BINDING_TASK: ComponentTask = ComponentTask {
            local_resources: &[ComponentTaskBinding::new("output", "motor_command")],
            ..CONCEPTUAL_SAFETY_COMPONENT.tasks[0]
        };
        const DIRECT_BINDING: ComponentDefinition = ComponentDefinition {
            tasks: &[DIRECT_BINDING_TASK],
            ..CONCEPTUAL_SAFETY_COMPONENT
        };
        const DIRECT_BINDING_DECLARATION: ComponentDeclaration = ComponentDeclaration {
            definition: &DIRECT_BINDING,
            ..CONCEPTUAL_SAFETY_CHANNEL
        };
        let error = expand(&F401_BOARD, &app(&[], &[DIRECT_BINDING_DECLARATION])).unwrap_err();
        assert!(error.to_string().contains("incompatible endpoint"));
    }
    ...
    #[test]
    fn exposed_component_port_is_bindable_but_private_state_is_not() {
        ...
        const READER: TaskDeclaration = READER_DEF
            .spawned_as("reader")
            .priority(1)
            .with_shared(&[resource("input").to_component("uart2", "rx")]);
        ...
        const SNOOP: TaskDeclaration = SNOOP_DEF
            .spawned_as("snoop")
            .priority(1)
            .with_local(&[resource("state").to_component("command_input", "decoder")]);
        let error = expand(&BOARD, &app(&[SNOOP], &[UART2_COMPONENT, COMMAND_INPUT])).unwrap_err();
        assert!(error.to_string().contains("port `decoder` is private"));
    }

    #[test]
    fn component_port_references_report_missing_inactive_and_incompatible_ports() {
        const READER_DEF: TaskDefinition =
            TaskDefinition::asynchronous("reader").with_shared(&[serial_rx("input")]);
        const MISSING_COMPONENT: TaskDeclaration = READER_DEF
            .spawned_as("missing_component")
            .priority(1)
            .with_shared(&[resource("input").to_component("missing", "rx")]);
        let error = expand(&BOARD, &app(&[MISSING_COMPONENT], &[])).unwrap_err();
        assert!(
            error
                .to_string()
                .contains("missing component instance `missing`")
        );

        const MISSING_PORT: TaskDeclaration = READER_DEF
            .spawned_as("missing_port")
            .priority(1)
            .with_shared(&[resource("input").to_component("uart2", "missing")]);
        let error = expand(&BOARD, &app(&[MISSING_PORT], &[UART2_COMPONENT])).unwrap_err();
        assert!(error.to_string().contains("has no port `missing`"));

        const DISABLED_UART: ComponentDeclaration = ComponentDeclaration {
            configuration: ComponentConfiguration::SerialPort(SerialProtocol::Disabled),
            ..UART2_COMPONENT
        };
        const INACTIVE_PORT: TaskDeclaration = READER_DEF
            .spawned_as("inactive_port")
            .priority(1)
            .with_shared(&[resource("input").to_component("uart2", "rx")]);
        let error = expand(&BOARD, &app(&[INACTIVE_PORT], &[DISABLED_UART])).unwrap_err();
        assert!(error.to_string().contains("port `rx` is inactive"));

        const WRONG_CAPABILITY: ComponentDeclaration = ComponentDeclaration {
            bindings: &[resource("rx").to_component("command_input", "rc_input")],
            ..COMMAND_INPUT
        };
        let error = expand(&BOARD, &app(&[], &[UART2_COMPONENT, WRONG_CAPABILITY])).unwrap_err();
        assert!(
            error
                .to_string()
                .contains("component port `command_input.rc_input` provides")
        );

        const WRONG_TASK_DEF: TaskDefinition = TaskDefinition::asynchronous("wrong_task")
            .with_shared(&[rc_input_observer_reader("input")]);
        const WRONG_TASK_CAPABILITY: TaskDeclaration = WRONG_TASK_DEF
            .spawned_as("wrong_task")
            .priority(1)
            .with_shared(&[resource("input").to_component("uart2", "rx")]);
        let expanded = expand(&BOARD, &app(&[WRONG_TASK_CAPABILITY], &[UART2_COMPONENT])).unwrap();
        let error = resolve::resolve(&BOARD, &expanded).unwrap_err();
        assert!(error.to_string().contains("component port `uart2.rx`"));
    }

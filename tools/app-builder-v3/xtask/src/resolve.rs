...
fn validate_software_capability(
    ...
    let target = binding
        .source_component_port
        .as_ref()
        .map(|port| format!("component port `{}.{}`", port.component, port.port))
        .unwrap_or_else(|| format!("software resource `{}`", declaration.id));
    if declaration.kind.capability() != capability {
        bail!(
            "task `{}` body resource `{task_resource}` requires {capability:?}, but {target} is incompatible",
            task.id,
        );
    }
    if capability.is_safety_channel() && task.safety_class != TaskSafetyClass::SafetyCritical {
        bail!(
            "task `{}` is {:?} and cannot own authoritative safety-channel {target}",
            task.id,
            task.safety_class,
        );
    }
        ...
        && declaration.owner_component.as_deref() != task.owner_component.as_deref()
    {
        ...
        if binding.source_component_port.is_some() {
            bail!(
                "task `{}` cannot bind private {target} owned by component `{owner}`",
                task.id
            );
        }
        bail!(
            "task `{}` cannot bind private resource `{}` owned by component `{owner}`",
            task.id,
            declaration.id
        );
    }
...
fn resolve_local_binding<'a>(
    ...
    match &binding.target {
        ...
        ExpandedResourceTarget::Software(resource_id) => {
            ...
            validate_software_capability(task, binding, capability, declaration)?;

...
pub enum ResourceTarget {
    ...
    /// Exposed software port declared by one component instance.
    ComponentPort {
        /// Component instance ID in the selected application.
        component: &'static str,

        /// Logical exposed resource or conceptual output ID.
        port: &'static str,
    },
}

impl ResourceTarget {
    /// Returns the selected hardware, software, or logical port identifier.
    ///
    /// Component ports are lowered to concrete generated identifiers during
    /// component expansion and cannot be rendered directly.
    pub const fn id(self) -> &'static str {
        match self {
            Self::Hardware(id) | Self::Software(id) => id,
            Self::ComponentPort { port, .. } => port,
        }
    }
}
...
impl UnboundResource {
    ...
    /// Binds the logical body field to an exposed component software port.
    pub const fn to_component(
        self,
        component: &'static str,
        port: &'static str,
    ) -> ResourceBinding {
        ResourceBinding {
            task_resource: self.task_resource,
            target: ResourceTarget::ComponentPort { component, port },
        }
    }
}
...
pub fn render(
    ...
    if declaration
        .local_resources
        .iter()
        .chain(declaration.shared_resources)
        .any(|binding| matches!(binding.target(), ResourceTarget::ComponentPort { .. }))
    {
        bail!(
            "task `{}` contains a component-port binding that must be expanded before rendering",
            declaration.id
        );
    }
...
fn validate_resource_list(task_id: &str, kind: &str, resources: &[ResourceBinding]) -> Result<()> {
    for (index, resource) in resources.iter().enumerate() {
        ...
        match resource.target() {
            ResourceTarget::Hardware(id) | ResourceTarget::Software(id) => {
                validate_identifier(id, &format!("task `{task_id}` {kind} concrete resource"))?
            }
            ResourceTarget::ComponentPort { component, port } => {
                validate_identifier(
                    component,
                    &format!("task `{task_id}` {kind} component instance"),
                )?;
                validate_identifier(port, &format!("task `{task_id}` {kind} component port"))?;
            }
        }
        if resources[..index]
            .iter()
...
"#;
    ...
    fn concrete_resource_namespace_is_part_of_the_binding() {
        ...
        let component_port = resource("rx").to_component("uart1", "rx");
        ...
        assert_eq!(component_port.task_resource(), "rx");
        assert_eq!(
            component_port.target(),
            ResourceTarget::ComponentPort {
                component: "uart1",
                port: "rx",
            }
        );
